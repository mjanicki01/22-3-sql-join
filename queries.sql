...
SELECT first_name, last_name, COUNT(owners.id) FROM owners JOIN vehicles ON owners.id = vehicles.owner_id GROUP BY first_name, last_name ORDER BY COUNT(owners.id);
...
SELECT first_name, last_name, COUNT(owners.id), ROUND(AVG(price)) AS avg_price
FROM owners
JOIN vehicles
ON owners.id = vehicles.owner_id 
GROUP BY first_name, last_name
HAVING COUNT(owners.id) >1 AND AVG(price) > 10000
